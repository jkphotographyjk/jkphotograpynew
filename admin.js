...
async function loadAllAdminData() {
    try {
        ...
        if (home) {
            ...
            const bannerPreview = document.getElementById('homeBannerPreview');

            if (heroTitle) heroTitle.value = home.hero_title || '';
            if (heroSubtitle) heroSubtitle.value = home.hero_subtitle || '';
            if (bannerPreview && home.banner_img) bannerPreview.src = home.banner_img;
        }
...
async function saveHomeSettings(event) {
    ...
    const file = document.getElementById('editHeroImgFile').files[0];
    const btn = document.getElementById('saveHomeBtn');

    try {
        if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> SAVING...'; }

        let banner_img = document.getElementById('homeBannerPreview').src;
        if (file) {
            banner_img = await uploadToSupabaseStorage(file, 'home');
            document.getElementById('homeBannerPreview').src = banner_img;
        }
        ...
        if (error) throw error;
        alert('Home page content saved to Supabase successfully!');
    } catch (e) {
        console.error(e);
        alert('Failed to save home page: ' + e.message);
    } finally {
        if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fa-solid fa-floppy-disk"></i> SAVE HOME PAGE TO SUPABASE'; }
    }
}
